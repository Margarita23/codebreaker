  ...
  describe Game do
    ...
    describe "#Code-breaker submit guess" do
      ...
      it "get 4 numbers from user" do
        @game.instance_variable_set(:@secret_code,[1,1,1,4])
        @game.instance_variable_set(:@guess,[1,4,1,1])
      ...
      describe "an exact match:" do
        ...
        describe "get the right results in the comparison of the alleged code and secret code" do
          ...
          it "get 1 plus in result" do
            @game.instance_variable_set(:@secret_code,[1,1,1,3])
            @game.instance_variable_set(:@guess,[3,3,3,3])
            @result = @game.check_submit_code
            expect(@result).to eq(["+"])
          end
          
          it "get 1 plus and 1 minus in result" do
            @game.instance_variable_set(:@secret_code,[5,1,1,3])
            @game.instance_variable_set(:@guess,[6,5,1,5])
            result = @game.check_submit_code
            expect(result).to eq(["+","-"])
          end
          
          it "get 2 pluses and 1 minus in result" do
            @game.instance_variable_set(:@secret_code,[5,1,1,3])
            @game.instance_variable_set(:@guess,[1,2,1,3])
            result = @game.check_submit_code
            expect(result).to eq(["+","+","-"])
          end
          
          it "get 4 minuses in result" do
            @game.instance_variable_set(:@secret_code,[1,2,2,1])
            @game.instance_variable_set(:@guess,[2,1,1,2])
            result = @game.check_submit_code
            expect(result).to eq(["-","-","-","-"])
          end
          
          it "get 3 pluses in result" do
            @game.instance_variable_set(:@secret_code,[1,1,3,1])
            @game.instance_variable_set(:@guess,[1,1,2,1])
            result = @game.check_submit_code
            expect(result).to eq(["+","+","+"])
          end
          
          it "get 2 minuses in result" do
          ...
          it "get 2 plus and 1 minus in result" do
            @game.instance_variable_set(:@secret_code,[3,1,1,6])
            @game.instance_variable_set(:@guess,[3,1,6,1])
            result = @game.check_submit_code
            expect(result).to eq(["+","+","-","-"])
          end
        end
